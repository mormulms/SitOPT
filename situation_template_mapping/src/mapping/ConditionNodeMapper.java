			...
			for (TConditionNode node : situation.getConditionNode()) {
				...
				for (TContextNode cnode : situation.getContextNode()) {
					for (TParent parent : cnode.getParent()) {
						if ((parent.getParentID() instanceof TConditionNode) && ((TConditionNode)parent.getParentID()).getId().equals(node.getId())) {
				...
				for (String object : objects) {
					...
					JSONObject nodeREDNode = NodeREDUtils.createNodeREDNode(
							situationTemplate.getId() + "." + node.getId() + object, node.getName() + " for " + object, "function", xCoordinate,
							Integer.toString(yCoordinate), situationTemplate.getId());
					...
					if (node instanceof TTimeNode) {
						...
						try {
							methodReturn = Nodes.class.getMethod("get" + timeNode.getOpType().toUpperCase() + "Node",
									String.class, String.class, String.class, String.class).invoke(null,
						...
						if (contextNodes != null && contextNodes.size() > 0) {
							try {
								java.lang.reflect.Method m = Nodes.class.getMethod(
										"get" + node.getOpType().toUpperCase() + "Node", contextNodes.getClass(),
										String.class, String.class, String.class);
								methodReturn = m.invoke(null, contextNodes, object, situationTemplate.getId(),
										sensorId);
						...
						try {
							methodReturn = Nodes.class
									.getMethod("get" + node.getOpType().toUpperCase() + "Node", List.class,
											String.class, String.class, String.class, String.class, String.class)
									.invoke(null, conditionValues, object, situationTemplate.getId(), sensorId, "0",
											"0");
						} catch (NoSuchMethodException | SecurityException | InvocationTargetException
								| IllegalAccessException e) {
							e.printStackTrace();
					...
					for (TParent parent : node.getParent()) {
						if (parent.getParentID() instanceof TConditionNode) {
							connections.add(situationTemplate.getId() + "."
									+ ((TConditionNode) parent.getParentID()).getId());
						} else if (parent.getParentID() instanceof TOperationNode) {
							connections.add(situationTemplate.getId() + "."
									+ ((TOperationNode) parent.getParentID()).getId());
						}
					}
