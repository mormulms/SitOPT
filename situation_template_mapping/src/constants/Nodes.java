...
public class Nodes {
	...
	private final static String accumulationString = "context.values = context.values || [];\n"
			...
			+ "      situations.push({'thing':array[index], 'timestamp':new Date(), 'situationtemplate':'%s' , 'occured':true, 'sensorvalues': []});\n"
			+ "    }\n"
			+ "  } else {\n"
			+ "    var array = [%s];\n"
			+ "    for (var index in array) {\n"
			+ "      situations.push({'thing':array[index], 'timestamp':new Date(), 'situationtemplate':'%s' , 'occured':false, 'sensorvalues': []});\n"
			+ "    }\n"
			+ "  }\n"
			+ "  context.values = null;\n"
	...
	private final static String timeString = "context.values = context.values || new Array();\n"
            ...
            + "  var array = [%s];"
            + "  for (var index in array) {\n"
            + "    msg.situation.push({'thing':array[index], 'timestamp':new Date(), 'situationtemplate':'A0' , 'occured':true});\n"
            + "  }\n"
            + "} else {\n"
            + "  var array = [%s];"
            + "  for (var index in array) {\n"
            + "    msg.situation.push({'thing':array[index], 'timestamp':new Date(), 'situationtemplate':'A0' , 'occured':false});\n"
            + "  }\n"
            + "}\n\n"
