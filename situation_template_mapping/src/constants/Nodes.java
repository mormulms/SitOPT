...
public class Nodes {
	...
	private final static String accumulationString = "context.values = context.values || [];\n"
			...
			+ "      msg.situation.push({'thing':array[index], 'timestamp':new Date(), 'situationtemplate':'%s' , 'occured':true});\n"
			+ "    }\n"
			+ "  } else {\n"
			+ "    var array = [%s];"
			+ "    for (var index in array) {\n"
			+ "      msg.situation.push({'thing':array[index], 'timestamp':new Date(), 'situationtemplate':'%s' , 'occured':false});\n"
			+ "    }\n"
			+ "  }\n"
			+ "  context.values = null;\n"
